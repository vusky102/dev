      ...
      <td>
        <table class="main">
          <caption>
            ...
            <xsl:choose>
              <xsl:when test="string-length($Amt) &lt; 10">
                <xsl:value-of select="concat(substring($Amt, 1, 3),' ',substring($Amt,string-length($Amt) - 5, 3), ',', substring($Amt, string-length($Amt) - 2))"/>
              </xsl:when>
              <xsl:when test="string-length($Amt) = 10">
                <xsl:value-of select="concat(substring($Amt, 1, 3),' ',substring($Amt, 4, 1), ',',substring($Amt,string-length($Amt) - 5, 3), ',', substring($Amt, string-length($Amt) - 2))"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="concat(substring($Amt, 1, 3), ' ',substring($Amt, 4, 2), ',', substring($Amt, string-length($Amt) - 5, 3), ',', substring($Amt, string-length($Amt) - 2))"/>
              </xsl:otherwise>
            </xsl:choose>
